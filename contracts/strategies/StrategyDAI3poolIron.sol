...
contract StrategyDAI3poolIron {
    ...
    address public constant ctrl = address(0xAB1c342C7bf5Ec5F02ADEA1c2270670bCa144CbB);
    address public constant cToken = address(0x7589C9E17BCFcE1Ccaa1f921196FDa177F0207Fc);
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        uint256 _bal = ICErc20(cToken).balanceOfUnderlying(address(this));
    ...
    function _withdrawAll() internal {
        uint256 _y3crv = ICErc20(cToken).balanceOfUnderlying(address(this));
    ...
    function balanceOfy3CRVin3CRV() public view returns (uint256) {
        uint256 _y3crv = balanceOfy3CRV().add(balanceOfy3CRVLent());
    ...
    function balanceOfy3CRVLent() public view returns (uint256) {
        (, uint256 tBal, , uint256 rate) = ICErc20(cToken).getAccountSnapshot(address(this));
    ...
    function drip() public isAuthorized {
        ...
        uint256 _u = ICErc20(cToken).balanceOfUnderlying(address(this));
        require(_u >= urn, "urn leak");
        uint256 _r = ((_p.mul(_u)).sub(p.mul(urn))).div(1e18);
        uint256 _s = _r.mul(strategistReward).div(DENOMINATOR);
        uint256 _t = _r.mul(treasuryFee).div(DENOMINATOR);
        uint256 _e = ICErc20(cToken).exchangeRateStored();
    ...
    function pop(uint256 _amt) public isAuthorized {
        drip();
