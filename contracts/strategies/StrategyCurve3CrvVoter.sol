...
contract StrategyCurve3CRVVoter {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    
    address constant public want = address(0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490);
    address constant public curve = address(0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7);
    address constant public pool = address(0xbFcF63294aD7105dEa65aA58F8AE5BE2D9d0952A);
    ...
    uint constant public baseMax = 10000;
    ...
    function withdraw(uint _amount) external {
        ...
        uint _fee = _amount.mul(withdrawalFee).div(baseMax);
    ...
    function harvest() public {
        ...
        if (_crv > 0) {
            uint _keepCRV = _crv.mul(keepCRV).div(baseMax);
        ...
        uint _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
            uint _fee = _want.mul(performanceFee).div(baseMax);
            uint _r = _want.mul(strategistReward).div(baseMax);
            IERC20(want).safeTransfer(strategist, _r);
            IERC20(want).safeTransfer(Controller(controller).rewards(), _fee);
            deposit();
        }
    }
